import { Field } from '@nestjs/graphql';
import { InputType } from '@nestjs/graphql';
import { ProjectCreateWithoutChurchInput } from './project-create-without-church.input';
import { Type } from 'class-transformer';
import { ProjectCreateOrConnectWithoutChurchInput } from './project-create-or-connect-without-church.input';
import { ProjectUpsertWithWhereUniqueWithoutChurchInput } from './project-upsert-with-where-unique-without-church.input';
import { ProjectCreateManyChurchInputEnvelope } from './project-create-many-church-input-envelope.input';
import { Prisma } from '@prisma/client';
import { ProjectWhereUniqueInput } from './project-where-unique.input';
import { ProjectUpdateWithWhereUniqueWithoutChurchInput } from './project-update-with-where-unique-without-church.input';
import { ProjectUpdateManyWithWhereWithoutChurchInput } from './project-update-many-with-where-without-church.input';
import { ProjectScalarWhereInput } from './project-scalar-where.input';

@InputType()
export class ProjectUncheckedUpdateManyWithoutChurchNestedInput {

    @Field(() => [ProjectCreateWithoutChurchInput], {nullable:true})
    @Type(() => ProjectCreateWithoutChurchInput)
    create?: Array<ProjectCreateWithoutChurchInput>;

    @Field(() => [ProjectCreateOrConnectWithoutChurchInput], {nullable:true})
    @Type(() => ProjectCreateOrConnectWithoutChurchInput)
    connectOrCreate?: Array<ProjectCreateOrConnectWithoutChurchInput>;

    @Field(() => [ProjectUpsertWithWhereUniqueWithoutChurchInput], {nullable:true})
    @Type(() => ProjectUpsertWithWhereUniqueWithoutChurchInput)
    upsert?: Array<ProjectUpsertWithWhereUniqueWithoutChurchInput>;

    @Field(() => ProjectCreateManyChurchInputEnvelope, {nullable:true})
    @Type(() => ProjectCreateManyChurchInputEnvelope)
    createMany?: ProjectCreateManyChurchInputEnvelope;

    @Field(() => [ProjectWhereUniqueInput], {nullable:true})
    @Type(() => ProjectWhereUniqueInput)
    set?: Array<Prisma.AtLeast<ProjectWhereUniqueInput, 'id'>>;

    @Field(() => [ProjectWhereUniqueInput], {nullable:true})
    @Type(() => ProjectWhereUniqueInput)
    disconnect?: Array<Prisma.AtLeast<ProjectWhereUniqueInput, 'id'>>;

    @Field(() => [ProjectWhereUniqueInput], {nullable:true})
    @Type(() => ProjectWhereUniqueInput)
    delete?: Array<Prisma.AtLeast<ProjectWhereUniqueInput, 'id'>>;

    @Field(() => [ProjectWhereUniqueInput], {nullable:true})
    @Type(() => ProjectWhereUniqueInput)
    connect?: Array<Prisma.AtLeast<ProjectWhereUniqueInput, 'id'>>;

    @Field(() => [ProjectUpdateWithWhereUniqueWithoutChurchInput], {nullable:true})
    @Type(() => ProjectUpdateWithWhereUniqueWithoutChurchInput)
    update?: Array<ProjectUpdateWithWhereUniqueWithoutChurchInput>;

    @Field(() => [ProjectUpdateManyWithWhereWithoutChurchInput], {nullable:true})
    @Type(() => ProjectUpdateManyWithWhereWithoutChurchInput)
    updateMany?: Array<ProjectUpdateManyWithWhereWithoutChurchInput>;

    @Field(() => [ProjectScalarWhereInput], {nullable:true})
    @Type(() => ProjectScalarWhereInput)
    deleteMany?: Array<ProjectScalarWhereInput>;
}
